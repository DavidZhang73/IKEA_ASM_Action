...
class _ObjectAnnotation(_AnnotationBase):
    ...
    def show(self):
        ...
        cv2.rectangle(img, (int(self.x), int(self.y)), (int(self.x + self.width), int(self.y + self.height)), color, 2)
        cv2.imshow(
...
class _SkeletonAnnotation(_AnnotationBase):
    ...
    def show(self):
        ...
        for edge in self._edge_list:
            ...
            cv2.line(img, from_point, to_point, color, 2)
        cv2.imshow(
...
class _ObjectAnnotations(_AnnotationsBase):
    ...
    def query(
        ...
        for annotation in annotation_list:
            if (
                (instance is None or annotation.instance == instance)
                and (score_min is None or (annotation.score is not None and annotation.score >= score_min))
                and (score_max is None or (annotation.score is not None and annotation.score <= score_max))
                and (label_id is None or annotation.label_id == label_id)
                and (color is None or color in annotation.color)
                and (x_min is None or annotation.x >= x_min)
                and (x_max is None or annotation.x <= x_max)
                and (y_min is None or annotation.y >= y_min)
                and (y_max is None or annotation.y <= y_max)
                and (width_min is None or annotation.width >= width_min)
                and (width_max is None or annotation.width <= width_max)
                and (height_min is None or annotation.height >= height_min)
                and (height_max is None or annotation.height <= height_max)
            ):
    ...
    def query(
        ...
        for annotation in annotation_list:
            if (
                ...
                and (label_id is None or annotation.label_id == label_id)
                and (color is None or color in annotation.color)
                and (n_point_min is None or annotation.n_point >= n_point_min)
                and (n_point_max is None or annotation.n_point <= n_point_max)
            ):
                if annotation.frame in result:
                    result[annotation.frame].append(annotation.raw_annotation)
                else:
    ...
    def query(
        ...
        for annotation in annotation_list:
            if (
                ...
                and (score_min is None or (annotation.score is not None and annotation.score >= score_min))
                and (score_max is None or (annotation.score is not None and annotation.score <= score_max))
                and (type_id is None or annotation.type_id == type_id)
                and (color is None or color in annotation.color)
                and (center_x_min is None or annotation.center_x >= center_x_min)
                and (center_x_max is None or annotation.center_x <= center_x_max)
                and (center_y_min is None or annotation.center_y >= center_y_min)
                and (center_y_max is None or annotation.center_y <= center_y_max)
            ):
                if annotation.frame in result:
                    result[annotation.frame].append(annotation.raw_annotation)
                else:
...
class _ActionAnnotations:
    ...
    def query(self, start=None, end=None, action_id=None, object_id=None, color=None, description=None):
        ...
        for action_annotation in self._action_annotation_list:
            if (
                (start is None or action_annotation.start >= start)
                and (end is None or action_annotation.end <= end)
                and (action_id is None or action_annotation.action_id == action_id)
                and (object_id is None or action_annotation.object_id == object_id)
                and (color is None or color in action_annotation.color)
                and (description is None or description in action_annotation.description)
            ):
